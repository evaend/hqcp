...
public class FormulaServiceImpl extends BaseService implements FormulaService{
	...
	public void insertForMula(Long qcOrgId, Long orgId, 
		...
		for (int j = 0; j < formulaTemplateMapper.selectAllFormulaTemplate().size(); j++) {
			...
			for (int i = 1; i <= FormulaTemplateDetailList.size(); i++) {
				//模板明细
				...
				if (i==1) {
					formulaDetail.setNumeratorValue(Long.valueOf(hospitalInfo.get("staffSum").toString()));
					formulaDetail.setDenominatorValue(Long.valueOf(hospitalInfo.get("planBedSum").toString()));
